...
export class FactoryDOMConfig extends Factory {
	...
	init () {
		...
		this.elements.forEach(conf => {
			...
			if (elementConfig.scroll.doScroll) {
				this.getScroller(this.baseConfig.scroll.buffer).addTarget(targetObject);
			}
			/* GROUP ---------------------------------------------------------*/
	...
	getElementFromSelector (selector) {
		let parentLevel = 0;
		while ((selector.startsWith && selector.startsWith('$parent::')) || _startsWith(selector, '$parent::')) {
...
export class FactoryDataAttributeConfig extends Factory {
	...
	init () {
		...
		this.elements.forEach(element => {
			try {
				...
				const elementConfig = this.parseDatasetToConfig(element.dataset, element);
				// CLICK TRIGGER
				const isSecondaryTrigger = !!element.dataset.triggersecondary;
				const trigger = new ClickTrigger(element, elementConfig, isSecondaryTrigger);
				// TARGET
				const targetElement = elementConfig.target;
				const target = targetElement.ghClickTrigger && targetElement.ghClickTrigger.addTrigger(trigger) || new Target(targetElement, elementConfig, trigger);
				// SCROLLING
	...
	parseDatasetToConfig (data, trigger) {
		...
		if (data.triggerreposition) {
			const reposition = this.getBooleanFromString(data.triggerreposition);
