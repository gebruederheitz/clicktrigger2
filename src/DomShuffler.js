...
export class DomShuffler {
	...
	determineDynamicTargetPosition (trigger) {
		...
		let insertionIndex = this.getNearestInsertionIndex(triggersInRow, triggerIndex, triggerCount);
	...
	getRowLength (trigger, triggerCount) {
		...
		const containerWidth = parseInt(window.getComputedStyle(trigger.parentElement).width, 10) || window.innerWidth;
